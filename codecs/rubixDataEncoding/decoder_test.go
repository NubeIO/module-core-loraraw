...
func TestSerialDataFull(t *testing.T) {
	// Skipped because serialMap has no entry for MDK_UO/UI/DO/DI (17-20), so
	// decodeData bails out for them without consuming any bits and the rest of
	// this buffer misaligns.
	//
	// Filling those entries in with {FIXEDPOINT, 0, 1, 3, 0} for UO/UI and
	// {FIXEDPOINT, 0, 1, 0, 0} for DO/DI makes this test pass exactly - all 17
	// fields, including the trailing end-of-buffer assertion that pins the
	// total bit count. It does change how pkg's TestEncryptedRubixPayload
	// decodes, but that fixture cannot arbitrate: its expectations already
	// contain "unknown-N" names, which only appear when a frame's 6-bit
	// metadata key has no entry at all, and with the entries added the same
	// frames still yield impossible values (lux-1 = 49152, UO-25 = 16384 on a
	// 0..1 point). Those frames decode to garbage either way - a separate,
	// pre-existing problem that needs the RDE spec or a hardware capture with
	// known point values to settle.
	//
	// Left unresolved deliberately: guessing at metadata would silently change
	// how real uplinks decode.
	t.Skip("no serialMap entry for MDK_UO/UI/DO/DI (17-20) - see comment above")
