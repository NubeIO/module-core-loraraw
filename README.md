...
- For cross-builds, use `build.bash` (wraps `Dockerfile.module`) or `Dockerfile`
  to produce amd64/armv7 binaries.
...
- `SerialOpen` requires `serial_port` and `serial_baud_rate`; failures set the
  network fault and retry after `re_iteration_time`.
...
- `logger/` — logrus setup.
- `build.bash`, `Dockerfile`, `Dockerfile.module` — containerized/cross builds.
